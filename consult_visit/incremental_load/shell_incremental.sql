...
from (select * from itcast_ods.web_chat_ems wce  where wce.starts_time='2025-05-31') wce join
(select * from itcast_ods.web_chat_text_ems where start_time='2025-05-31') wcte
on wce.id=wcte.id;
...
select
  ...
  substr(wce.create_time,9,2) as dayinfo
from 
  (select * from itcast_ods.web_chat_ems where starts_time='${dateStr}') wce 
join 
  (select * from itcast_ods.web_chat_text_ems where start_time='${dateStr}') wcte 
on wce.id = wcte.id;
"

${HIVE_HOME}/bin/hive -e "${sqlStr}" -S